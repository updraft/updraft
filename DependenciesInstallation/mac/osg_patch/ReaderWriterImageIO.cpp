...
//            bitmap_info = kCGImageAlphaNone;
        ...
        case 16:
        {
            image_data = calloc(the_width * 2, the_height);
            internal_format = GL_LUMINANCE16;
            pixel_format = GL_LUMINANCE16;
            data_type = GL_UNSIGNED_SHORT;
            
            bytes_per_row = the_width * 2;
            color_space = CGColorSpaceCreateDeviceGray();
            bits_per_component = 16;
            bitmap_info = 0;
            //OSG_WARN << "bitmap_info: " << bitmap_info << std::endl;
            //OSG_WARN << "alpha_info: " << alpha_info << std::endl;
            //bitmap_context = CGBitmapContextCreate(image_data, the_width, the_height, bits_per_component, bytes_per_row, color_space, alpha_info);
            break;
        }
        case 32:
        {
...
//            bitmap_info = kCGImageAlphaPremultipliedFirst;
    ...
    switch(bits_per_pixel)
    {
        case 16:
        {
            int image_size_in_bytes = the_height * the_width * 2;
            GLubyte *pixels = (GLubyte *)image_data;
            for(int j=0; j<image_size_in_bytes; j+=2)
            {
                GLubyte active_byte = pixels[j];
                pixels[j] = pixels[j+1];
                pixels[j + 1] = active_byte;
            }
        }
        default:
        {
